...
interface IFeeApprover {
    ...
    function feePercentX10() external view returns (uint);

    function setTokenUniswapPair(address _tokenUniswapPair) external;

    function setKernelTokenAddress(address _kernelTokenAddress) external;
...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    bool private adminEnableEmergencyWithdraw;
    bool private stoppedLPEvent;
    ...
    function canClaimLP(address _owner) public  view returns (uint256) { 
        ethContributed[_owner].mul(LPperETHUnit).div(1e18);
    }
    ...
    string public liquidityGenerationParticipationAgreement = "I agree that the developers and affiliated parties of the Kerenl team are not responsible for your funds";

    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        require(contractStartTimestamp < block.timestamp, "Event didnt start");
        require(liquidityGenerationOngoing(), "Event over");
        require(stoppedLPEvent == false, "Event was stop");
        return contractStartTimestamp.add(5 days).sub(block.timestamp);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        if (stoppedLPEvent == false)
            return contractStartTimestamp < block.timestamp && contractStartTimestamp.add(5 days) > block.timestamp;
        else 
            return false;
    }
    ...
    // activated if we find a bug or summury investment will be lower then 500 either;
    function activateEmergencyWithdraw() public onlyOwner {
        adminEnableEmergencyWithdraw = true;
    }

    function stopLiquidityEvent() public onlyOwner {
        stoppedLPEvent = true;
    }

    // every investor can recive their ETH if admin activated emergency withdraw;
    ...
    uint256 public timeToTransactLP;
    ...
    function addLiquidityToUniswapKERNELxWETHPair() public {
        require(adminEnableEmergencyWithdraw == false, "Liquidity revert to investors");
        require(contractStartTimestamp < block.timestamp, "Liquidity generation was start");
        ...
        timeToTransactLP = block.timestamp.add(1 hours);
        emit Transfer(address(this), initDevAddr, _totalSupply.div(100).mul(7));
    }
    ...
    function addLiquidity(bool agreesToTermsOutlinedInLiquidityGenerationParticipationAgreement) public payable {
        ...
        require(msg.value < 200 ether, "Too large investment");
    ...
    function claimLPTokens() public {
        ...
        require(timeToTransactLP < block.timestamp, "Hold 60 minutes, after generation LP");
        IUniswapV2Pair pair = IUniswapV2Pair(tokenUniswapPair);
    ...
    function _mint(address account, address _initDevAddr, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _beforeTokenTransfer(address(0), account, amount);

        _totalSupply = _totalSupply.add(amount);
        _balances[account] = _balances[account].add(amount.div(100).mul(93));
        _balances[_initDevAddr] =  _balances[initDevAddr].add(amount.div(100).mul(7));
...
// KernelToken with Governance.
contract KERNEL is NBUNIERC20 {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "KERNEL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "KERNEL::delegateBySig: invalid nonce");
        require(now <= expiry, "KERNEL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "KERNEL::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KERNEL tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "KERNEL::_writeCheckpoint: block number exceeds 32 bits");
